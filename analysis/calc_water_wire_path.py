...
def get_water_cylinder_inds(frame_xyz_prot, water_o, ref_crd):
    ...
    #squared radii of the water oxygens from the protein z axis
    ...
    ATP_ind = 0
    if frame_xyz_prot[cyl_o_inds[0]] not in ref_crd:
        print("warning: ATP not found in reference coordinates; check that the first index in cyl_o_inds corresponds to ATP")
        sys.exit(0)


    return cyl_o_inds, c_water_ind, m_water_ind, ATP_ind
...
def get_sqdist_matrix(coords_all, indices):
    ...
    max_sqdist = 2 #nm^2; distance above which waters are not considered connected
    distances_zeroed = np.where(distances > max_sqdist, 0, distances)
...
def mst_2_distances(predecessors, o_dist_mat, cyl_o_inds, centered_coords, target_ind, printcommands = False):
    ...
    return path_dists, xyz, nodelist_structure_inds


#TODO can we remove frame as an argument to make this mdtraj independent?
def get_water_pc_ATP(centered_coords, water_o, atp_inds, vertices, printcommands = False):

    #t1 = time.time()

    atp_ref_crds = centered_coords[atp_inds]

    #get cylinder waters
    cyl_o_inds, c_water_ind, m_water_ind, ATP_ind = get_water_cylinder_inds(centered_coords, water_o, atp_ref_crds)
    ...
    dist_matrix, predecessors = shortest_path(mst, directed=False, indices=ATP_ind, return_predecessors=True)
    ...
    m_path_dists, m_xyz, m_nodelist_structure_inds = mst_2_distances(predecessors, o_dist_mat, cyl_o_inds, centered_coords, m_water_ind, printcommands = False)
    c_path_dists, c_xyz, c_nodelist_structure_inds = mst_2_distances(predecessors, o_dist_mat, cyl_o_inds, centered_coords, c_water_ind, printcommands = False)
    
    dist_matrix, predecessors = shortest_path(mst, directed=False, indices=m_water_ind, return_predecessors=True)
    cm_path_dists, cm_xyz, cm_nodelist_structure_inds = mst_2_distances(predecessors, o_dist_mat, cyl_o_inds, centered_coords, c_water_ind, printcommands = False)

    return m_path_dists, c_path_dists, cm_path_dists
...
#TODO can we remove frame as an argument to make this mdtraj independent?
def get_water_pc(centered_coords, water_o, atp_inds, vertices, printcommands = False):
...
def save_wire_pc(trj_path, top_path, protein, savefilename, queries):

    upperpath = "/home/jonathan/Documents/grabelab/aac1-nt-transport/metadynamics/run4/analysis_output/"
    ...
    if check_queries:
        for q in queries:
            ...
            # else:
            #     print(f"{len(trj.top.select(q))} atoms selected for query {q}")

    water_o = trj.top.select(combined_queries)
    atp_inds = trj.top.select("resname ATP and not element H")

    print(len(water_o), "waters selected")
    ...
    for fi in range(0, trj.n_frames):
        ...
        #t11 = time.time()
        m_path_dists, c_path_dists, cm_path_dists = get_water_pc_ATP(trj_xyz_prot[fi], water_o, atp_inds, vertices, printcommands = False)
        #path_dists, xyz, nodelist, path_class = get_water_pc(trj_xyz_prot[fi], water_o, atp_inds, vertices, printcommands = False)
        #print(f"main time = {t22-t11}")
        # print(f"frame {fi} took {t2-t1} seconds")

        #paths.append(path_class)

        maxdist_c = np.sqrt(max(c_path_dists))
        pcs_c.append(maxdist_c)
        maxdist_m = np.sqrt(max(m_path_dists))
        pcs_m.append(maxdist_m)
        maxdist_cm = np.sqrt(max(cm_path_dists))
        pcs_cm.append(maxdist_cm)
        #t2 = time.time()
    ...
    np.save(_savefilename+"-gaps_c" + ".npy", pcs_c)
    np.save(_savefilename+"-gaps_m" + ".npy", pcs_m)
    #print(paths)
    paths = np.array(paths).astype("uint8")
    np.save(_savefilename+"-paths" + ".npy", paths)

    return pcs_c, pcs_m, pcs_cm, paths
